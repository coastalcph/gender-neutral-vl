#!/bin/bash
#SBATCH --job-name=albef4m_cc3m_neutral
...
ckpt=${OUTS_DIR}/Pretrain_CC3M_neutral/albef_4m/checkpoint_00.pth 
...
cd $CODE_DIR
CUDA_VISIBLE_DEVICES="0,1,2,3" python -m torch.distributed.run --nproc_per_node=4 --master_port $(($RANDOM + $RANDOM)) Retrieval.py \
	--distributed \
    --config $configs \
    --checkpoint $ckpt \
    --output_dir $output \
