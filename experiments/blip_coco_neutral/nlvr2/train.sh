...
name=blip_coco_neutral
task=NLVR
configs=configs/${task}.yaml
ckpt=${OUTS_DIR}/Pretrain_COCO_neutral/blip/checkpoint_00.pth
...
cd $CODE_DIR
CUDA_VISIBLE_DEVICES="0,1,2,3" python -m torch.distributed.run --nproc_per_node=4 --master_port $(($RANDOM + $RANDOM)) train_nlvr.py \
    --distributed \
    --config $configs \
    --checkpoint $ckpt \
    --output_dir $output \
