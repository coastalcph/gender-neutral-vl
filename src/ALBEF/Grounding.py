...
def train(
    ...
    for i, (image, text, idx) in enumerate(
        ...
        text_input = tokenizer(
            text, padding="longest", max_length=30, return_tensors="pt"
        ).to(device)
...
def val(model, data_loader, tokenizer, device, gradcam_mode, block_num):
    ...
    result = []
    for image, text, ref_ids in metric_logger.log_every(
        ...
        if gradcam_mode == "itm":
            ...
            with torch.no_grad():
                ...
                grads = (
                    grads[:, :, :, 1:]
                    .clamp(min=0)
                    .reshape(image.size(0), 12, -1, 24, 24)
                    * mask
                )
            ...
            with torch.no_grad():
                grad = (
                    model.visual_encoder.blocks[block_num]
...
def main(args, config):
    ...
    for epoch in range(0, max_epoch):
        ...
        results = collect_result(
            result, args.result_dir, "epoch%d" % epoch, is_json=False, is_list=True
        )

        if utils.is_main_process():
            grounding_acc = grounding_eval(
                results, dets, cocos, refer, alpha=0.5, mask_size=24
            )
