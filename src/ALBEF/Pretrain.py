...
def train(
    ...
    for i, (image, text) in enumerate(
        ...
        if args.steps_scheduler:
            scheduler.step(epoch * len(data_loader) + i)
        elif epoch == 0 and i % step_size == 0 and i <= warmup_iterations:
...
def main(args, config):
    ...
    if args.use_neutral_data:
        datasets = [create_dataset("pretrain_neutral", config)]
    else:
    ...
    if args.steps_scheduler:
        config["schedular"]["epochs"] = int(
            config["schedular"]["epochs"] * len(data_loader)
        )
        config["schedular"]["warmup_epochs"] = int(
            config["schedular"]["warmup_epochs"] * len(data_loader)
        )
    ...
    for epoch in range(start_epoch, max_epoch):
        if not args.steps_scheduler and epoch > 0:
