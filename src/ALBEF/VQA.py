...
def train(
    ...
    for i, (image, question, answer, weights, n) in enumerate(
        ...
        answer_input = tokenizer(answer, padding="longest", return_tensors="pt").to(
            device
        )
...
def evaluation(model, data_loader, tokenizer, device, config, args):
    ...
    answer_input = tokenizer(answer_list, padding="longest", return_tensors="pt").to(
        device
    )
    ...
    for n, (image, question, question_id) in enumerate(
        ...
        question_input = tokenizer(question, padding="longest", return_tensors="pt").to(
            device
        )
        ...
        for ques_id, topk_id, topk_prob in zip(question_id, topk_ids, topk_probs):
            if not isinstance(ques_id, str):
                ques_id = int(ques_id.item())
                ques_key = "question_id"
                pred_key = "answer"
            else:
                ques_key = "questionId"
                pred_key = "prediction"
            ...
            result.append(
                {
                    ques_key: str(ques_id),
                    pred_key: data_loader.dataset.answer_list[topk_id[pred]],
                }
            )
...
def main(args, config):
    ...
    vqa_result = evaluation(model, test_loader, tokenizer, device, config, args)
    result_file = save_result(
        vqa_result,
        args.result_dir,
        "vqa_result_epoch%d" % epoch,
