...
class ALBEF(nn.Module):
    def __init__(
        self, text_encoder=None, tokenizer=None, config=None, temp=0.07, init_deit=True
    ):
        super().__init__()
    ...
    def forward(self, image, text, alpha=0, do_mlm=True, do_itm=True):
        with torch.no_grad():
        ...
        if self.distill:
            ...
            image_feat_all = concat_all_gather(image_feat).T
            text_feat_all = concat_all_gather(text_feat).T
            sim_targets = torch.zeros((bs, image_feat_all.shape[1])).to(image.device)
            sim_targets.fill_diagonal_(1)
        ...
        if not do_itm:
            return (
                torch.zeros([], device=loss_ita.device),
                loss_ita,
                torch.zeros([], device=loss_ita.device),
            )
        ...
        if not do_mlm:
            return torch.zeros([], device=loss_ita.device), loss_ita, loss_itm
