...
from data.pretrain_dataset import pretrain_dataset, pretrain_neutral_dataset
...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset == "pretrain":
    ...
    elif dataset == "pretrain_neutral":
        dataset = pretrain_neutral_dataset(
            config["train_file"],
            config["train_neutral_file"],
            transform_train,
            max_words=config["max_words"],
            p_neutral_caption=config["p_neutral_caption"],
            batch_size=config["batch_size"],
            num_epochs=config["neutral_epochs"],
        )
        return dataset
        ...
        train_dataset = coco_karpathy_train(
            transform_train, config["image_root"], config["train_file"]
        )
        val_dataset = coco_karpathy_retrieval_eval(
            transform_test, config["image_root"], config["val_file"]
        )
        test_dataset = coco_karpathy_retrieval_eval(
            transform_test, config["image_root"], config["test_file"]
        )
        return train_dataset, val_dataset, test_dataset
