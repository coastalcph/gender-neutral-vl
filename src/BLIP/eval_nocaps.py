...
def main(args, config):
    ...
    if args.distributed:
        ...
        samplers = create_sampler(
            [val_dataset, test_dataset], [False, False], num_tasks, global_rank
        )
    else:
        samplers = [None, None]
    ...
    val_result_file = save_result(
        val_result, args.result_dir, "val", remove_duplicate="image_id"
    )
    test_result = evaluate(model_without_ddp, test_loader, device, config)
    test_result_file = save_result(
        test_result, args.result_dir, "test", remove_duplicate="image_id"
    )
