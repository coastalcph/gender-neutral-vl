...
def train(model, data_loader, optimizer, epoch, device, config, args):
    ...
    for i, (image, caption) in enumerate(
        ...
        elif epoch == 0 and args.steps_scheduler:
            e = i / len(data_loader) * 20  # BLIP originally pretrained for 20 epochs
            step_lr_schedule(
                optimizer,
                e,
                config["init_lr"],
...
def main(args, config):
    ...
    if args.use_neutral_data:
        datasets = [create_dataset("pretrain_neutral", config, min_scale=0.2)]
    else:
    ...
    if args.steps_scheduler:
        config["warmup_steps"] = int(config["warmup_steps"] * len(data_loader))
    ...
    for epoch in range(start_epoch, config["max_epoch"]):
        ...
        train_stats = train(model, data_loader, optimizer, epoch, device, config, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_neutral_data", default=False, action="store_true")
    parser.add_argument("--steps_scheduler", default=False, action="store_true")
