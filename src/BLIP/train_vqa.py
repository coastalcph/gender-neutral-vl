        ...
        if config["inference"] == "generate":
            ...
            for answer, ques_id in zip(answers, question_id):
                if not isinstance(ques_id, str):
                    ...
                    ques_key = "questionId"
                    pred_key = "prediction"
                result.append({ques_key: str(ques_id), pred_key: answer})
            ...
            for ques_id, answer_id in zip(question_id, answer_ids):
                if not isinstance(ques_id, str):
                    ...
                    ques_key = "questionId"
                    pred_key = "prediction"
                result.append({ques_key: ques_id, pred_key: answer_list[answer_id]})
    return result
...
def main(args, config):
    ...
    for epoch in range(0, config["max_epoch"]):
        if not args.evaluate:
            ...
            train_stats = train(model, train_loader, optimizer, epoch, device, args)
    ...
    vqa_result = evaluation(model_without_ddp, test_loader, device, config, args)
