...
def parse_args():
    ...
    parser.add_argument(
        "--features_path",
        default="datasets/conceptual_caption/imgfeats",
        type=str,
...
def main():
    ...
    if args.from_pretrained:
        ...
        model = BertForVLPreTraining.from_pretrained(
            args.from_pretrained, config=config, default_gpu=default_gpu, from_hf=True
        )
        # Resize type embeddings
        model.bert.embeddings.token_type_embeddings = model._get_resized_embeddings(
            model.bert.embeddings.token_type_embeddings, type_vocab_size
        )
    ...
    for epoch_id in range(start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataset):
            ...
            masked_loss_t, masked_loss_v, pair_match_loss = model(
                input_ids,
                image_feat,
                image_loc,
                segment_ids,
