...
class CocoLoaderTrain(object):
    ...
    def __init__(
        ...
        lmdb_file = os.path.join(features_path, "train_feat.lmdb")
        ...
        neutral_caption_path = os.path.join(annotations_path, "gender-neutral/caption_train.json")
    ...
    def __iter__(self):
        ...
        for batch in self.ds.get_data():
            input_ids, input_mask, segment_ids, lm_label_ids, is_next, image_feat, image_loc, \
                image_cls, obj_labels, obj_confs, attr_labels, attr_confs, image_attrs, \
            image_label, image_mask, masked_label, image_id = batch
...
class CocoLoaderVal(object):
    ...
    def __init__(
        ...
        lmdb_file = os.path.join(features_path, "val_feat.lmdb")
        print("Loading from %s" % lmdb_file)

        caption_path = os.path.join(annotations_path, "caption_valid.json")
        neutral_caption_path = os.path.join(annotations_path, "gender-neutral/caption_valid.json")
...
class BertPreprocessBatch(object):
    def __init__(
        ...
        self.num_caps = int(data_size/5)
        self.captions = list(json.load(open(caption_path, "r")).values())
        self.neutral_captions_dict = json.load(open(neutral_caption_path, "r"))
    ...
    def __call__(self, data):
        ...
        attr_confs, attr_labels, image_location_wp, caption_id, image_feature_wp, \
        image_h, image_w, obj_confs, obj_labels, image_id_cap = data.values()

        image_id = image_id_cap.split("_")[0]  # image_id_cap = image_id + _ + caption_id
        ...
        num_boxes = int(image_location_wp.shape[-2])
        ...
        caption = ""  # does not matter because self.visualization = False
        # Convert image_id to our annotations format
        # image_id = str(int(image_id.split("2014_")[-1]))
        caption, label = self.random_cap(caption, image_id, caption_id)
        ...
        self.p_neutral_caption = self.p_neutral_caption + \
                                 (1 - self.p_neutral_caption) * (self.step / self.num_train_steps)
    ...
    def random_cap(self, caption, image_id, caption_id):
        """
        ...
        rnd = random.random()
        if self.objective != 2 and rnd > 0.5:
            caption = self.get_random_caption()
            label = 1
        else:
            if rnd <= self.p_neutral_caption:
                caption = self.neutral_captions_dict[image_id][caption_id]
            label = 0
