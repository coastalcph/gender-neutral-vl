...
def _load_dataset(dataroot, name, annotations_jsonpath):
    ...
    if name == "train" or name == "val":
        ...
        items_path = os.path.join(dataroot, "testdev_all_questions.json")
    ...
    def __init__(
        ...
        if not os.path.exists(cache_path):
            self.entries = _load_dataset(
                dataroot, LXMERT_SPLIT_MAP[split], annotations_jsonpath
            )
            ...
            if type(self.entries) == pd.DataFrame:
                logger.debug("Reformatting data into list of dictionaries")
                self.entries = self.entries.to_dict("records")
