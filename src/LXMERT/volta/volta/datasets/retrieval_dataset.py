...
def _load_annotations(annotations_jsonpath, task):
    if task == "RetrievalCOCO":
        entries = []
        imgid2entry = {}
        data = json.load(open(annotations_jsonpath, "r"))
        for image_id, sents in data.items():
...
class RetrievalDataset(Dataset):
    def __init__(
        ...
        os.makedirs(
            os.path.join("/".join(annotations_jsonpath.split("/")[:-1]), "cache"),
            exist_ok=True,
        )
        ...
        if self._split == "train":
            ...
            if task == "RetrievalCOCO":
                img_ids_in_validation = json.load(
                    open(
                        os.path.join(
                            dataroot, "hard_negative_img_id_in_valid_split.json"
                        )
                    )
                )
                self.train_image_list = [
                    ...
                    if str(item["image_id"]) in img_ids_in_validation["img_id"]
                ]
                print(f"{len(self.train_image_list)} training images.")
            self.train_imgId2pool = {
                int(imageId): i for i, imageId in enumerate(self.train_image_list)
            }
        self.task = task
    ...
    def tokenize(self):
        ...
        for entry in self._entries:
            ...
            tokens = (
                [tokens[0]] + tokens[1:-1][: self._max_seq_length - 2] + [tokens[-1]]
            )
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            if img_id2 != image_id:
                ...
                segment_ids = torch.from_numpy(
                    np.array(segment_ids + [0] * len(padding))
                )
                entry2 = {
                    "token": tokens,
        ...
        sample_random = True
        if (
            self._split == "train" and image_id in self.train_imgId2pool
        ):  # Added because there're 5K extra images in entries (lxmert train)
            # random hard caption.
            if self.task == "RetrievalCOCO":
                rand_img_id_pool = self.train_hard_pool[self.train_imgId2pool[image_id]]
                try:
                    intersect = list(
                        set(rand_img_id_pool).intersection(set(self.train_image_list))
                    )
                    assert len(intersect) > 0
                    while True:
                        pool_img_idx = int(
                            intersect[np.random.randint(1, len(intersect))]
                        )
                        if pool_img_idx in self.train_image_list:
                            img_id4 = self.train_image_list[pool_img_idx]
                            entry4 = self._entries[
                                random.choice(self.imgid2entry[img_id4])
                            ]
                            break
                except AssertionError:
                    print("Random hard caption was not sample. Let's do it random")
            else:
                rand_img_id_pool = self.train_hard_pool[self.train_imgId2pool[image_id]]
                pool_img_idx = int(
            ...
            while True:
                ...
                if img_id4 != image_id:
                    ...
                    input_mask = torch.from_numpy(
                        np.array(input_mask + [0] * len(padding))
                    )
                    segment_ids = torch.from_numpy(
                        np.array(segment_ids + [0] * len(padding))
                    )
...
def _load_annotationsVal(
    annotations_jsonpath, task
) -> (List[int], List[Dict[str, Any]]):
    entries = {}
    caption_entries = []
    if task == "RetrievalCOCO" and annotations_jsonpath.endswith(".json"):
        data = json.load(open(annotations_jsonpath, "r"))
        # Build an index which maps image id with a list of caption annotations.
        for image_id, sents in data.items():
            image_id = int(image_id)
            ...
            for annotation in reader:
                ...
                if task == "RetrievalWIT":
                    caption_entries.append(
                        {
                            "caption": annotation["caption_reference_description"],
                            "image_id": image_id,
                        }
                    )
                else:
                    for sentences in annotation["sentences"]:
                        caption_entries.append(
                            {"caption": sentences, "image_id": image_id}
                        )
    entries = [*entries]
    return entries, caption_entries
...
class RetrievalDatasetVal(Dataset):
    def __init__(
        ...
        self.max_num_images = self.num_images // self.num_subiters + int(
            self.num_images % self.num_subiters > 0
        )
        ...
        self.features_all = np.zeros(
            (len(self._image_entries), self._max_region_num, 2048)
        )
        self.spatials_all = np.zeros(
            (len(self._image_entries), self._max_region_num, self._num_locs)
        )
    ...
    def __getitem__(self, index):
        ...
        image_entries = self._image_entries[
            self.max_num_images * (image_idx) : self.max_num_images * (image_idx + 1)
        ]
    ...
    def __iter__(self):
        for ix, batch in enumerate(self.ds.get_data()):
            ...
            if self.add_global_imgfeat == "first":
                ...
                image_feats = np.concatenate(
                    [np.expand_dims(g_image_feats, axis=1), image_feats], axis=1
                )
                image_feats = np.array(image_feats, dtype=np.float32)

                g_loc = [0, 0, 1, 1] + [1] * (self.num_locs - 4)
                g_image_locs = np.repeat(
                    np.array([g_loc], dtype=np.float32), batch_size, axis=0
                )
                image_locs = np.concatenate(
                    [np.expand_dims(g_image_locs, axis=1), image_locs], axis=1
                )
                ...
                g_image_feats = np.sum(image_feats, axis=1) / sum_count
                image_feats = np.concatenate(
                    [image_feats, np.expand_dims(g_image_feats, axis=1)], axis=1
                )
                image_feats = np.array(image_feats, dtype=np.float32)
                ...
                image_locs = np.concatenate(
                    [image_locs, np.expand_dims(g_image_locs, axis=1)], axis=1
                )
            ...
            input_ids = np.stack(
                [input_ids1s, input_ids1s, input_ids3s, input_ids4s], axis=1
            )
            input_ids = torch.tensor(input_ids, dtype=torch.long)
            input_mask = np.stack(
                [input_mask1s, input_mask1s, input_mask3s, input_mask4s], axis=1
            )
            input_mask = torch.tensor(input_mask, dtype=torch.long)
            segment_ids = np.stack(
                [segment_ids1s, segment_ids1s, segment_ids3s, segment_ids4s], axis=1
            )
...
class BertPreprocessBatch(object):
    ...
    def __call__(self, item):
        ...
        if self.norm_embeddings:
            ...
            image_location = image_location / np.linalg.norm(
                image_location, 2, 1, keepdims=True
            )
        ...
        tokens_caption1 = (
            [tokens_caption1[0]]
            + tokens_caption1[1:-1][: self.seq_len - 2]
            + [tokens_caption1[-1]]
        )
        tokens_caption3 = self.tokenizer.encode(caption3)
        tokens_caption3 = (
            [tokens_caption3[0]]
            + tokens_caption3[1:-1][: self.seq_len - 2]
            + [tokens_caption3[-1]]
        )
        tokens_caption4 = self.tokenizer.encode(caption4)
        tokens_caption4 = (
            [tokens_caption4[0]]
            + tokens_caption4[1:-1][: self.seq_len - 2]
            + [tokens_caption4[-1]]
        )
