...
class BertImagePredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.decoder_dict = nn.ModuleDict({
            ...
            if config.visual_target_weights.get(ix) is not None
        })
...
class BertForVLPreTraining(BertPreTrainedModel):
    ...
    def forward_no_loss(self, text_input_ids, text_attention_mask, visual_inputs_ids, bounding_boxes):

        # Deactivate loss computation - no needed here
        self.visual_target_weights = {}
        self.qa_weight = None
        prediction_scores_t, _, _, _, _, encoded_layers_t, encoded_layers_v = self.forward(
            input_ids=text_input_ids,
            image_feat=visual_inputs_ids,
            image_loc=bounding_boxes,
            attention_mask=text_attention_mask,
            output_all_encoded_layers=True
        )

        # vision_output = encoded_layers_v[-1].contiguous()
        # text_output = encoded_layers_t[-1].contiguous()

        # order = OrderedDict()
        # order.last_hidden_state = torch.cat((vision_output, text_output), dim=1)  # same as vision_output
        # order[0] = order.last_hidden_state
        # order.hidden_states = None  # doesnot matter
        # order.attentions = None  # doesnot matter
