...
from volta.volta.datasets import DatasetMapTrain, DatasetMapEval
from volta.volta.datasets._image_features_reader import ImageFeaturesH5Reader
...
def ForwardModelsVal(config, task_cfg, device, task_id, batch, model, criterion):
    ...
    with torch.no_grad():
        vil_prediction, vision_prediction, linguisic_prediction, _ = model(
            question, features, spatials, task_id, segment_ids, input_mask, image_mask
        )
    ...
    return float(loss), float(batch_score), batch_size, vil_logit, target
...
def ForwardModelsTrain(config, task_cfg, device, task_id, batch, model, criterion):
    ...
    if task_cfg[task_id]["process"] in ["dialog"]:
        ...
        features = (
            ...
            .unsqueeze(1)
            .expand(
                batch_size, nround, num_options, max_num_bbox, config.v_feature_size
            )
            .contiguous()
            .view(-1, max_num_bbox, config.v_feature_size)
        )
...
def LoadDataset(args, config, task_cfg, task_id, split="trainval"):
    ...
    train_annotations_jsonpath = (
        args.train_annotations_jsonpath or task_cfg[task]["train_annotations_jsonpath"]
    )
    val_annotations_jsonpath = (
        args.val_annotations_jsonpath or task_cfg[task]["val_annotations_jsonpath"]
    )
    ...
    if data_format == "serialized_lmdb":
        ...
        features_reader1 = (
            ImageFeaturesH5Reader(feats_h5path1, config, args.in_memory)
            if feats_h5path1 != ""
            else None
        )
        features_reader2 = (
            ImageFeaturesH5Reader(feats_h5path2, config, args.in_memory)
            if feats_h5path2 != ""
            else None
        )
        if "train" in split:
            if args.train_features_lmdbpath:
                features_reader = ImageFeaturesH5Reader(
                    args.train_features_lmdbpath, config, args.in_memory
                )
            else:
        ...
        if "val" in split:
            eval_batch_size = args.eval_batch_size or task_cfg[task].get(
...
def EvaluatingModel(
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
            ...
            results.append(
                {
                    ...
                    # Following isn't valid because consider just 1 target
                    # but VQA provides more than one target with different weights (scores)
                    # "target": dataloader.dataset.label2ans[torch.argmax(target[i]).item()]
                }
            )
            ...
            results.append(
                {
                    ...
                    "target": dataloader.dataset.label2ans[
                        torch.argmax(target[i]).item()
                    ],
                }
            )
        ...
        for i in range(preds.size(0)):
            results.append(
                {
                    "sentence": dataloader.dataset.entries[question_id[i].item()][
                        "sentence"
                    ],
