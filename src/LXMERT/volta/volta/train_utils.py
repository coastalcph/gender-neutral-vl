...
class tbLogger(object):
    def __init__(self, log_dir, txt_dir, task_names, task_ids, task_num_iters,
                 gradient_accumulation_steps, save_logger=False, txt_name="out.txt"):
    ...
    def step_val_CC(self, epochId, iter_id, masked_loss_t, masked_loss_v, next_sentence_loss, task_id, batch_size, split):
...
def resume(path, model, optimizer, scheduler, tb_logger):
    ...
    if path != "" and os.path.exists(path):
        ...
        del checkpoint
        logger.info(f"Resume training... epoch:{start_epoch}, step:{global_step}, best score:{best_score}")
