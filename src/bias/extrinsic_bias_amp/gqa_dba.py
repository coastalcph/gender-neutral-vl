...
def preprocess_gqa_df_to_dbamp_at_input(
    gender_counter,
    top_objects: List,
    qdf: pd.DataFrame,
    ...
    if pred_df is not None:
        ...
        pred_df = pred_df.loc[pred_df["questionId"].isin(list(sub_qdf["question_id"]))]
        print(len(pred_df))

    # Fill-in TASK values
    for obj_idx, obj in enumerate(top_objects):
        # label__lemma and label_lemma has no white spaces
        obj = obj.replace(" ", "")
        if pred_df is None:  # Only qdf provided to preprocess ground truth
            ...
            # q = np.where(sub_qdf['label__lemma'].str.match(obj))
            q = np.where(sub_qdf["label_"].str.match(obj))
            for q_idx in q[0]:
        ...
        # PREDICTIONS
        if pred_df is not None:
            # q = np.where(pred_df['prediction_lemma'].str.match(obj))
            q = np.where(pred_df["prediction"].str.match(obj))
            for q_idx in q[0]:
...
# Gender in Questions
qdf = pd.read_pickle(f"{input_dir}/valid_qa_gender.pkl")
...
labels = json.load(
    open(
        "../../preprocessing/top_objects/labels_in_gender_text_gqa_train.json",
        "rb",
    )
)
...
qdf = qdf[qdf.label__lemma.isin(top_objects)]
question_ids = sorted(qdf.question_id.tolist())
gender_counter = qdf["q_gender"].value_counts()
print(
    f"Total: {len(qdf)} questions-answer pairs with gender info and target 'objects' with FREQ > 100"
)
...
if not sweep:
    for modelname, path_ in RESULTS.items():
        ...
        if modelname.startswith("albef") or modelname.startswith("blip"):
            # VALIDATION
            model = model[model.questionId.isin(qdf.question_id)]

        if "gender" not in model.columns:
            # Add gender to predictions
            qdf.sort_values("question_id", ascending=True, inplace=True)
            model.sort_values("questionId", ascending=True, inplace=True)
            model["img_id"] = qdf["img_id"].values
            # gender
            model["gender"] = ""
            for qid, img_id, gender in qdf.loc[
                qdf["q_gender"] != "", ["question_id", "img_id", "q_gender"]
            ].values:
                model.loc[
                    (model["questionId"] == qid) & (model["img_id"] == img_id), "gender"
                ] = gender
        qdf.sort_values("question_id", ascending=True, inplace=True)
        model.sort_values("questionId", ascending=True, inplace=True)
        ...
        ) = preprocess_gqa_df_to_dbamp_at_input(
            gender_counter, top_objects, qdf=qdf, attr_label_names=attribute_label_names
        )

        _, _, task_model_preds, _ = preprocess_gqa_df_to_dbamp_at_input(
            gender_counter,
            top_objects,
            qdf,
            model,
        ...
        print("\n" + modelname.upper(), dba_gender_to_obj_lxm)
    ...
    for modelname in [
        ...
        for seed in [0, 23, 42, 56, 92]:
            ...
            if "gender" not in model.columns:
                # VALIDATION
                qdf = qdf[qdf.question_id.isin(model.questionId)]
                model = model[model.questionId.isin(qdf.question_id)]
                model.drop_duplicates("questionId", inplace=True)
                ...
                for qid, img_id, gender in qdf.loc[
                    ...
                    model.loc[
                        (model["questionId"] == qid) & (model["img_id"] == img_id),
                        "gender",
                    ] = gender
            qdf.sort_values("question_id", ascending=True, inplace=True)
            model.sort_values("questionId", ascending=True, inplace=True)
