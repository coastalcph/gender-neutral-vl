# Source: https://github.com/princetonvisualai/directional-bias-amp/blob/main/directional_biasamp.py
# Paper: https://arxiv.org/abs/2102.12594
# Last access: September 2022
...
def biasamp_task_to_attribute(
    task_labels: np.ndarray,
    attribute_labels: np.ndarray,
    attribute_preds: np.ndarray,
    ...
    keep_indices = np.array(
        list(
            set(np.where(np.sum(task_labels_train, axis=1) > 0)[0]).union(
                set(np.where(np.sum(attribute_labels_train, axis=1) > 0)[0])
            )
        )
    )
    task_labels_train, attribute_labels_train = (
        task_labels_train[keep_indices],
        attribute_labels_train[keep_indices],
    )
    ...
    for a in range(num_a):
        for t in range(num_t):
            a_cond_t[a][t] = np.mean(
                attribute_labels[:, a][np.where(task_labels[:, t] == 1)[0]]
            )
            ahat_cond_t[a][t] = np.mean(
                attribute_preds[:, a][np.where(task_labels[:, t] == 1)[0]]
            )
    ...
    if names is not None:
        assert (
            len(names) == 2
        ), "Names should be a list of the task names and attribute names"
        task_names, attribute_names = names
        assert (
            len(task_names) == num_t and len(attribute_names) == num_a
        ), "The number of names should match both the number of tasks and number of attributes"
        ...
        for i in sorted_indices[::-1][:10]:
            ...
            print(
                "{0} - {1}: {2:.4f}".format(
                    attribute_names[a], task_names[t], values[a][t]
                )
            )
    return val, values


def biasamp_attribute_to_task(
    task_labels: np.ndarray,
    attribute_labels: np.ndarray,
